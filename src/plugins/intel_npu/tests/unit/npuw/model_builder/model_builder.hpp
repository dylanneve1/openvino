...
/// FFN activation type
enum class FFNType {
    SWIGLU,  ///< SwiGLU (LLaMA style)
    GELU     ///< GELU (GPT/BERT style)
};
...
enum class RoPEType {
    HALF_ROTATION,  ///< Standard LLaMA: split in half, rotate
    INTERLEAVED     ///< GPT-NeoX: interleave odd/even elements
};
...
/// Configuration for tokenizer generation
struct TokenizerConfig {
    ...
    int64_t pad_token_id = 256;
    int64_t unk_token_id = 257;
    int64_t bos_token_id = 258;
    int64_t eos_token_id = 259;
};

/// Result from building tokenizer models
struct TokenizerResult {
    std::shared_ptr<ov::Model> tokenizer;      ///< nullptr if libopenvino_tokenizers not found
    std::shared_ptr<ov::Model> detokenizer;     ///< nullptr if libopenvino_tokenizers not found
};
...
struct LLMConfig {
    ...
    size_t get_kv_heads() const { return num_kv_heads == 0 ? num_heads : num_kv_heads; }
    size_t get_past_len() const { return context_len > seq_len ? context_len - seq_len : 0; }
};
...
public:
    ...
    std::shared_ptr<ov::Model> get_model_with_repeated_blocks_and_results(
        std::size_t repetitions, const std::vector<std::size_t>& block_indices);
    ...
    std::shared_ptr<ov::Model> get_model_with_multi_output_repeating_blocks(
        std::size_t repetitions, bool last_block_has_direct_result);
    ...
    ov::Output<ov::Node> make_linear(const ov::Output<ov::Node>& input, size_t out_features,
    ...
    ov::Output<ov::Node> make_norm(const ov::Output<ov::Node>& input, size_t hidden_size,
                                   const std::string& name,
    ...
    /// Build tokenizer and detokenizer models using openvino_tokenizers extension.
    /// Returns {nullptr, nullptr} if libopenvino_tokenizers is not available.
    TokenizerResult build_tokenizer(const TokenizerConfig& config = TokenizerConfig{});

    /// Write tokenizer JSON config files (tokenizer_config.json, special_tokens_map.json)
    static void write_tokenizer_configs(const std::filesystem::path& dir, const TokenizerConfig& config);
...
private:
    ov::Output<ov::Node> make_layer_norm(const ov::Output<ov::Node>& input, size_t hidden_size,
                                         const std::string& name, ov::element::Type precision, float eps);
    ov::Output<ov::Node> make_rms_norm(const ov::Output<ov::Node>& input, size_t hidden_size,
                                       const std::string& name, ov::element::Type precision, float eps);
    ov::Output<ov::Node> make_swiglu_ffn(const ov::Output<ov::Node>& input, size_t hidden_size,
                                         size_t intermediate_size, const std::string& name,
                                         ov::element::Type precision, WeightFormat weight_format);
    ov::Output<ov::Node> make_gelu_ffn(const ov::Output<ov::Node>& input, size_t hidden_size,
                                       size_t intermediate_size, const std::string& name,
                                       ov::element::Type precision, WeightFormat weight_format);
    ov::Output<ov::Node> make_half_rotation_rope(const ov::Output<ov::Node>& input,
                                                  const ov::Output<ov::Node>& position_ids,
                                                  size_t head_dim, size_t max_position,
                                                  const std::string& name, ov::element::Type precision);
    ...
    ov::Output<ov::Node> make_weight(const std::string& name, size_t rows, size_t cols,
