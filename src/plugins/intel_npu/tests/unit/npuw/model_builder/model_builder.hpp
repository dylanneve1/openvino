...
using WeightFn = std::function<ov::Output<ov::Node>(
    const std::string&, size_t, size_t, ov::element::Type)>;

/// Norm functor: construction captures hidden_size, precision, eps;
/// call takes (input, name) -> output
using NormFn = std::function<ov::Output<ov::Node>(
    const ov::Output<ov::Node>&, const std::string&)>;

/// FFN functor: construction captures hidden_size, intermediate_size, precision, weight_fn;
/// call takes (input, name) -> output
using FFNFn = std::function<ov::Output<ov::Node>(
    const ov::Output<ov::Node>&, const std::string&)>;
...
struct LayerNorm {
    ...
    LayerNorm(size_t hs, ov::element::Type prec = ov::element::f32, float e = 1e-5f)
        : hidden_size(hs), precision(prec), eps(e) {}
...
struct HalfRotationRoPE {
    ...
    HalfRotationRoPE(size_t hd, size_t mp, ov::element::Type prec = ov::element::f32)
        : head_dim(hd), max_position(mp), precision(prec) {}

    ov::Output<ov::Node> operator()(const ov::Output<ov::Node>& input,
                                     const ov::Output<ov::Node>& position_ids, const std::string& name) const;
};
...
struct SwiGLU {
    ...
    SwiGLU(size_t hs, size_t is, ov::element::Type prec, WeightFn wf)
        : hidden_size(hs), intermediate_size(is), precision(prec), weight_fn(std::move(wf)) {}
