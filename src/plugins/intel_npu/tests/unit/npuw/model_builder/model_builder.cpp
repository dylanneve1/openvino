...
std::shared_ptr<ov::op::v0::Parameter> ModelBuilder::parameter(
    ov::element::Type type, const ov::PartialShape& shape, const std::string& name) {
...
std::shared_ptr<ov::op::v0::Result> ModelBuilder::result(
    const ov::Output<ov::Node>& output, const std::string& name) {
...
ov::Output<ov::Node> ModelBuilder::make_weight(const std::string& name, size_t rows, size_t cols,
                                                WeightFormat format, ov::element::Type compute_precision) {
    switch (format) {
        case WeightFormat::FP32: {
            auto weight = ov::opset11::Constant::create(
                compute_precision, ov::Shape{rows, cols},
        ...
        case WeightFormat::FP16: {
            ...
            auto weight = ov::opset11::Constant::create(
                ov::element::f16, ov::Shape{rows, cols},
                std::vector<float>(rows * cols, 0.01f));
            weight->set_friendly_name(name);
            m_nodes.push_back(weight);
        ...
        case WeightFormat::INT8: {
            ...
            auto weight = ov::opset11::Constant::create(
                ov::element::i8, ov::Shape{rows, cols},
                std::vector<int8_t>(rows * cols, 1));
            weight->set_friendly_name(name);
            m_nodes.push_back(weight);
            ...
            auto scale = ov::opset11::Constant::create(
                ov::element::f16, ov::Shape{rows, 1},
                std::vector<float>(rows, 0.01f));
        ...
        case WeightFormat::INT4: {
            ...
            // Matches DCOFF SymmNoZP pattern for i4 weights (like real Llama INT4)
            auto weight = ov::opset11::Constant::create(
                ov::element::i4, ov::Shape{rows, cols},
                std::vector<int8_t>(rows * cols, 1));
            weight->set_friendly_name(name);
            m_nodes.push_back(weight);
...
ov::Output<ov::Node> ModelBuilder::make_linear(const ov::Output<ov::Node>& input,
    ...
    if (add_bias) {
        auto bias = ov::opset11::Constant::create(
            precision, ov::Shape{out_features}, std::vector<float>(out_features, 0.0f));
...
ov::Output<ov::Node> ModelBuilder::make_layer_norm(const ov::Output<ov::Node>& input,
                                                    ...
                                                    float eps) {
    auto weight = ov::opset11::Constant::create(
        precision, ov::Shape{hidden_size}, std::vector<float>(hidden_size, 1.0f));
    weight->set_friendly_name(name + ".weight");
    m_nodes.push_back(weight);

    auto bias = ov::opset11::Constant::create(
        precision, ov::Shape{hidden_size}, std::vector<float>(hidden_size, 0.0f));
    bias->set_friendly_name(name + ".bias");
    m_nodes.push_back(bias);
...
ov::Output<ov::Node> ModelBuilder::make_multihead_reshape(const ov::Output<ov::Node>& input,
                                                           ...
                                                           size_t head_dim,
                                                           const std::string& name) {
    auto shape = ov::opset11::Constant::create(ov::element::i64, ov::Shape{4},
...
ov::Output<ov::Node> ModelBuilder::make_repeat_kv(const ov::Output<ov::Node>& kv,
    ...
    auto broadcast_shape = std::make_shared<ov::opset11::Concat>(
        ov::OutputVector{batch_kv_heads, n_rep_const, seq_head_dim}, 0);
    ...
    auto broadcasted = std::make_shared<ov::op::v3::Broadcast>(unsqueezed, broadcast_shape,
    ...
    auto new_shape = ov::opset11::Constant::create(ov::element::i64, ov::Shape{4},
        std::vector<int64_t>{0, static_cast<int64_t>(num_heads), -1,
...
ModelBuilder::KVCacheResult ModelBuilder::make_kv_cache_concat(const ov::Output<ov::Node>& current_kv,
    ...
    auto variable = std::make_shared<ov::op::util::Variable>(
        ov::op::util::VariableInfo{var_shape, precision, name});
    ...
    auto num_heads_const = ov::opset11::Constant::create(ov::element::i64, ov::Shape{1},
    ...
    auto init_shape = std::make_shared<ov::opset11::Concat>(
        ov::OutputVector{batch_dim->output(0), num_heads_const->output(0),
                         zero_seq->output(0), head_dim_const->output(0)}, 0);
    ...
    auto concat = std::make_shared<ov::opset11::Concat>(
        ov::OutputVector{beam_gather->output(0), current_kv}, 2);
...
ov::Output<ov::Node> ModelBuilder::make_half_rotation_rope(const ov::Output<ov::Node>& input,
    ...
    auto cos_table = ov::opset11::Constant::create(
        precision, ov::Shape{max_position, head_dim},
        std::vector<float>(max_position * head_dim, 0.5f));
    cos_table->set_friendly_name(name + ".cos_table");
    m_nodes.push_back(cos_table);

    auto sin_table = ov::opset11::Constant::create(
        precision, ov::Shape{max_position, head_dim},
        std::vector<float>(max_position * head_dim, 0.5f));
    ...
    auto head_dim_const = ov::opset11::Constant::create(ov::element::i64, ov::Shape{1}, {static_cast<int64_t>(head_dim)});
...
ov::Output<ov::Node> ModelBuilder::make_interleaved_rope(const ov::Output<ov::Node>& input,
    ...
    auto reshape_5d = ov::opset11::Constant::create(ov::element::i64, ov::Shape{5},
        std::vector<int64_t>{0, 0, 0, half_dim, 2});
    ...
    auto reshape_4d = ov::opset11::Constant::create(ov::element::i64, ov::Shape{4},
...
LayerResult ModelBuilder::make_attention_block(const ov::Output<ov::Node>& input,
    ...
    if (config.use_position_ids && position_ids.get_node() != nullptr) {
        q_for_trans = make_rope(q_reshaped, position_ids, head_dim, config.context_len,
                                prefix + "q_rope", config.rope_type, prec);
        k_for_trans = make_rope(k_reshaped, position_ids, head_dim, config.context_len,
                                prefix + "k_rope", config.rope_type, prec);
    }
    ...
    auto reshape_shape = ov::opset11::Constant::create(ov::element::i64, ov::Shape{3},
...
ov::Output<ov::Node> ModelBuilder::make_ffn_block(const ov::Output<ov::Node>& input,
                                                   const LLMConfig& config,
                                                   const std::string& prefix) {
    auto normed = make_norm(input, config.hidden_size, prefix + "post_attention_layernorm",
                            config.norm_type, config.precision);

    auto ffn_out = make_ffn(normed, config.hidden_size, config.intermediate_size,
                            prefix + "mlp", config.ffn_type, config.precision, config.weight_format);
...
LayerResult ModelBuilder::make_decoder_layer(const ov::Output<ov::Node>& input,
    ...
    auto attn_result = make_attention_block(input, config, layer_idx, prefix, position_ids, batch_source, beam_idx, attention_mask);
...
ov::Output<ov::Node> ModelBuilder::make_embedding(const ov::Output<ov::Node>& input_ids,
                                                   ...
                                                   ov::element::Type precision) {
    auto weight = ov::opset11::Constant::create(
        precision, ov::Shape{vocab_size, hidden_size},
...
std::shared_ptr<ov::Model> ModelBuilder::build_llm(const LLMConfig& config) {
    ...
    for (size_t layer = 0; layer < config.num_layers; ++layer) {
        auto layer_result = make_decoder_layer(current, config, layer, position_ids_output,
    ...
    auto logits = make_lm_head(final_norm, config.hidden_size, config.vocab_size,
                               "lm_head", prec, config.lm_head_format);
...
constexpr int32_t TOKENIZER_MAX_LENGTH = 8192;

// Load a shared library, returning nullptr on failure
std::shared_ptr<void> try_load_shared_object(const std::string& path) {
#ifdef _WIN32
    HMODULE handle = LoadLibraryA(path.c_str());
    if (!handle) return nullptr;
    return std::shared_ptr<void>(handle, [](void* h) { if (h) FreeLibrary(static_cast<HMODULE>(h)); });
#else
    void* handle = dlopen(path.c_str(), RTLD_LAZY);
    if (!handle) return nullptr;
    return std::shared_ptr<void>(handle, [](void* h) { if (h) dlclose(h); });
#endif
}

void* get_symbol(const std::shared_ptr<void>& shared_object, const char* symbol_name) {
#ifdef _WIN32
    return reinterpret_cast<void*>(GetProcAddress(static_cast<HMODULE>(shared_object.get()), symbol_name));
#else
    dlerror();
    void* sym = dlsym(shared_object.get(), symbol_name);
    return dlerror() ? nullptr : sym;
#endif
}

// Try to find and load libopenvino_tokenizers
std::shared_ptr<void> find_tokenizers_library() {
#ifdef _WIN32
    const std::string lib_name = "openvino_tokenizers.dll";
#elif defined(__APPLE__)
    const std::string lib_name = "libopenvino_tokenizers.dylib";
#else
    const std::string lib_name = "libopenvino_tokenizers.so";
#endif

    // 1. Try LD_LIBRARY_PATH / system search (just the bare name)
    auto lib = try_load_shared_object(lib_name);
    if (lib) return lib;

    // 2. Try relative to libopenvino (common install layout)
    //    Search LD_LIBRARY_PATH entries explicitly
#ifndef _WIN32
    const char* ld_path = std::getenv("LD_LIBRARY_PATH");
    if (ld_path) {
        std::istringstream paths(ld_path);
        std::string dir;
        while (std::getline(paths, dir, ':')) {
            auto candidate = dir + "/" + lib_name;
            lib = try_load_shared_object(candidate);
            if (lib) return lib;
        }
    }
#endif
...
// Maps each byte 0-255 to a unicode codepoint, then encodes as UTF-8
std::vector<std::string> bytes_to_unicode() {
    // Printable ASCII + Latin-1 supplement ranges that map directly
    std::vector<int> bs;
...
// Build the inverse mapping: unicode string -> byte value
std::unordered_map<std::string, uint8_t> unicode_to_bytes_map() {
    auto b2u = bytes_to_unicode();
    std::unordered_map<std::string, uint8_t> result;
    for (int i = 0; i < 256; ++i) {
        result[b2u[i]] = static_cast<uint8_t>(i);
    }
    return result;
}

// Apply GPT-2 unicode-to-byte decoding on a token string
std::vector<uint8_t> apply_unicode_to_bytes(const std::string& token,
                                             const std::unordered_map<std::string, uint8_t>& u2b) {
    std::vector<uint8_t> res;
    size_t i = 0;
    while (i < token.size()) {
        unsigned char lead = static_cast<unsigned char>(token[i]);
        int char_len = 1;
        ...
        auto it = u2b.find(utf8_char);
        if (it != u2b.end()) {
            res.push_back(it->second);
        } else {
            // Not in byte mapping, keep raw bytes
            for (char c : utf8_char) res.push_back(static_cast<uint8_t>(c));
        }
        i += char_len;
    }
    return res;
...
// Generate dummy vocabulary: 256 byte tokens + special tokens + filler
struct DummyVocab {
    std::vector<std::string> tokens;        // Original token strings
    std::vector<std::vector<uint8_t>> decoded;  // Byte-decoded tokens
    std::vector<int32_t> special_token_ids;
};

DummyVocab generate_dummy_vocab(size_t vocab_size, const TokenizerConfig& config) {
    auto b2u = bytes_to_unicode();
    auto u2b = unicode_to_bytes_map();

    DummyVocab vocab;
    vocab.tokens.reserve(vocab_size);

    // Tokens 0-255: byte tokens (GPT-2 encoded)
    for (int i = 0; i < 256; ++i) {
        vocab.tokens.push_back(b2u[i]);
    }

    // Token 256: pad, 257: unk, 258: bos, 259: eos
    vocab.tokens.push_back(config.pad_token);  // 256
    vocab.tokens.push_back(config.unk_token);  // 257
    vocab.tokens.push_back(config.bos_token);  // 258
    vocab.tokens.push_back(config.eos_token);  // 259

    // Mark special tokens
    vocab.special_token_ids = {256, 257, 258, 259};

    // Fill remaining with dummy tokens
    for (size_t i = 260; i < vocab_size; ++i) {
        std::ostringstream oss;
        oss << "madeupword" << std::setw(6) << std::setfill('0') << i;
        vocab.tokens.push_back(oss.str());
    }

    // Decode all tokens to bytes
    vocab.decoded.reserve(vocab.tokens.size());
    for (const auto& tok : vocab.tokens) {
        vocab.decoded.push_back(apply_unicode_to_bytes(tok, u2b));
    }

    return vocab;
}

// Encode strings as {begins, ends, chars} OV constants
ov::OutputVector create_string_constant(const std::vector<std::vector<uint8_t>>& input_strings) {
    ...
    auto const_begins = std::make_shared<ov::op::v0::Constant>(
        ov::element::i32, ov::Shape{begins.size()}, begins)->output(0);
    auto const_ends = std::make_shared<ov::op::v0::Constant>(
        ov::element::i32, ov::Shape{ends.size()}, ends)->output(0);
    auto const_chars = std::make_shared<ov::op::v0::Constant>(
        ov::element::u8, ov::Shape{chars.size()}, chars)->output(0);

    return {const_begins, const_ends, const_chars};
}


// Create ragged begins/ends from batch dimension
ov::OutputVector add_ragged_dimension(const ov::OutputVector& inputs) {
    auto input_shape = std::make_shared<ov::op::v3::ShapeOf>(inputs[0], ov::element::i32);
    auto const_zero = std::make_shared<ov::op::v0::Constant>(ov::element::i32, ov::Shape{}, 0);
    auto const_one = std::make_shared<ov::op::v0::Constant>(ov::element::i32, ov::Shape{}, 1);
    auto batch_size = std::make_shared<ov::op::v8::Gather>(input_shape, const_zero, const_zero);
    auto batch_plus_one = std::make_shared<ov::op::v1::Add>(batch_size, const_one);
    auto ragged_begins = std::make_shared<ov::op::v4::Range>(const_zero, batch_size, const_one, ov::element::i32)->output(0);
    auto ragged_ends = std::make_shared<ov::op::v4::Range>(const_one, batch_plus_one, const_one, ov::element::i32)->output(0);

    ov::OutputVector res = {ragged_begins, ragged_ends};
    res.insert(res.end(), inputs.begin(), inputs.end());
    return res;
}

// Quote special characters for regex
...
TokenizerResult ModelBuilder::build_tokenizer(const TokenizerConfig& config) {
    // Try to load openvino_tokenizers
    auto lib = find_tokenizers_library();
    if (!lib) {
        return {nullptr, nullptr};
    }

    auto* create_func = reinterpret_cast<FactoryCreateType>(
        get_symbol(lib, "create_tokenizer_node"));
    if (!create_func) {
        return {nullptr, nullptr};
    }

    auto vocab = generate_dummy_vocab(config.vocab_size, config);

    // ===== TOKENIZER MODEL =====
    auto tokenizer_input = std::make_shared<ov::op::v0::Parameter>(
        ov::element::string, ov::PartialShape{ov::Dimension::dynamic()});

    ov::OutputVector outputs = create_func("StringTensorUnpack", {tokenizer_input}, {});
    outputs = add_ragged_dimension(outputs);

    // SpecialTokensSplit: build regex from special tokens
    std::string special_re;
    for (size_t i = 0; i < vocab.special_token_ids.size(); ++i) {
        if (i > 0) special_re += "|";
        special_re += quote_meta(vocab.tokens[vocab.special_token_ids[i]]);
    }
    ov::Tensor special_re_tensor(ov::element::u8, {special_re.size()});
    std::memcpy(special_re_tensor.data<uint8_t>(), special_re.data(), special_re.size());
    auto const_special_re = std::make_shared<ov::op::v0::Constant>(special_re_tensor);

    ov::OutputVector split_inputs = outputs;
    split_inputs.push_back(const_special_re->output(0));
    outputs = create_func("SpecialTokensSplit", split_inputs, {});

    // RegexSplit with default patterns
    std::vector<std::string> split_patterns = {
        "[\\p{P}\\$\\+<=>\\^~\\|]+",
    ...
    for (const auto& pattern : split_patterns) {
        ov::Tensor re_tensor(ov::element::u8, {pattern.size()});
        std::memcpy(re_tensor.data<uint8_t>(), pattern.data(), pattern.size());
        auto const_re = std::make_shared<ov::op::v0::Constant>(re_tensor);

        outputs.push_back(const_re->output(0));
        outputs = create_func("RegexSplit", outputs,
                              {{"behaviour", std::string("isolate")},
                               {"invert", false},
                               {"max_splits", -1}});
    }

    // BPETokenizer: build inputs
    // First 5 outputs from regex chain are ragged_begins, ragged_ends, str_begins, str_ends, chars
    ov::OutputVector bpe_inputs(outputs.begin(), outputs.begin() + 5);

    // Add vocab constant
    auto vocab_const = create_string_constant(vocab.decoded);
    bpe_inputs.insert(bpe_inputs.end(), vocab_const.begin(), vocab_const.end());

    // Add empty merges (character-level tokenization)
    std::vector<std::vector<uint8_t>> empty_merges;
    auto left_merges = create_string_constant(empty_merges);
    auto right_merges = create_string_constant(empty_merges);
    bpe_inputs.insert(bpe_inputs.end(), left_merges.begin(), left_merges.end());
    bpe_inputs.insert(bpe_inputs.end(), right_merges.begin(), right_merges.end());

    // Add special tokens for BPE
    std::vector<std::vector<uint8_t>> special_tok_strs;
    for (auto id : vocab.special_token_ids) {
        special_tok_strs.push_back(vocab.decoded[id]);
    }
    auto special_const = create_string_constant(special_tok_strs);
    bpe_inputs.insert(bpe_inputs.end(), special_const.begin(), special_const.end());

    auto const_special_indices = std::make_shared<ov::op::v0::Constant>(
        ov::element::i32, ov::Shape{vocab.special_token_ids.size()}, vocab.special_token_ids);
    bpe_inputs.push_back(const_special_indices->output(0));

    int32_t cache_capacity = std::max<int32_t>(
        static_cast<int32_t>(config.vocab_size * 0.2f), 20000);

    outputs = create_func("BPETokenizer", bpe_inputs,
    ...
    auto max_len_const = std::make_shared<ov::op::v0::Constant>(
        ov::element::i32, ov::Shape{}, TOKENIZER_MAX_LENGTH);
    auto ends_minus_begins = std::make_shared<ov::op::v1::Subtract>(outputs[1], outputs[0]);
    auto clamped_len = std::make_shared<ov::op::v1::Minimum>(ends_minus_begins, max_len_const);
    outputs[0] = std::make_shared<ov::op::v1::Subtract>(outputs[1], clamped_len)->output(0);

    // RaggedToDense with left padding disabled
    auto reduce_axis = std::make_shared<ov::op::v0::Constant>(ov::element::i32, ov::Shape{1}, 0);
    auto ends_minus_begins2 = std::make_shared<ov::op::v1::Subtract>(outputs[1], outputs[0]);
    auto max_len_batch = std::make_shared<ov::op::v1::ReduceMax>(ends_minus_begins2, reduce_axis, false);

    ov::OutputVector rtd_inputs = outputs;
    rtd_inputs.push_back(max_len_batch->output(0));
    auto pad_value = std::make_shared<ov::op::v0::Constant>(ov::element::i32, ov::Shape{}, 0);
    rtd_inputs.push_back(pad_value->output(0));

    outputs = create_func("RaggedToDense", rtd_inputs,
                          {{"pad_right", false}, {"pad_max_length", false}});

    // Convert to i64
    outputs[0] = std::make_shared<ov::op::v0::Convert>(outputs[0], ov::element::i64)->output(0);
    outputs[1] = std::make_shared<ov::op::v0::Convert>(outputs[1], ov::element::i64)->output(0);
    outputs[0].get_tensor().add_names({"input_ids"});
    outputs[1].get_tensor().add_names({"attention_mask"});

    auto tokenizer_model = std::make_shared<ov::Model>(
        outputs, ov::ParameterVector{tokenizer_input}, "tokenizer");

    // ===== DETOKENIZER MODEL =====
    auto detok_input = std::make_shared<ov::op::v0::Parameter>(
        ov::element::i64, ov::PartialShape{ov::Dimension::dynamic(), ov::Dimension::dynamic()});

    auto detok_vocab_const = create_string_constant(vocab.decoded);
    ov::OutputVector detok_outputs = {detok_input};
    detok_outputs.insert(detok_outputs.end(), detok_vocab_const.begin(), detok_vocab_const.end());

    // Add special token IDs for skip
    auto special_ids_const = std::make_shared<ov::op::v0::Constant>(
        ov::element::i32, ov::Shape{vocab.special_token_ids.size()}, vocab.special_token_ids);
    auto slice_zero = std::make_shared<ov::op::v0::Constant>(ov::element::i32, ov::Shape{1}, 0);
    auto slice_max = std::make_shared<ov::op::v0::Constant>(
        ov::element::i32, ov::Shape{1}, std::numeric_limits<int32_t>::max());
    auto slice_one = std::make_shared<ov::op::v0::Constant>(ov::element::i32, ov::Shape{1}, 1);
    auto sliced_skips = std::make_shared<ov::op::v8::Slice>(
        special_ids_const, slice_zero, slice_max, slice_one)->outputs();
    detok_outputs.insert(detok_outputs.end(), sliced_skips.begin(), sliced_skips.end());

    // VocabDecoder -> FuzeRagged -> UTF8Validate -> StringTensorPack
    detok_outputs = create_func("VocabDecoder", detok_outputs, {});

    ov::OutputVector fuse_inputs(detok_outputs.begin(), detok_outputs.end() - 1);
    auto fused = create_func("FuzeRagged", fuse_inputs, {});
    fused.insert(fused.end(), detok_outputs.end() - 1, detok_outputs.end());

    auto validated = create_func("UTF8Validate", fused, {{"replace_mode", true}});
    auto packed = create_func("StringTensorPack", validated, {});
    packed[0].get_tensor().add_names({"string_output"});

    auto detokenizer_model = std::make_shared<ov::Model>(
        packed, ov::ParameterVector{detok_input}, "detokenizer");

    return {tokenizer_model, detokenizer_model};
}

void ModelBuilder::write_tokenizer_configs(const std::filesystem::path& dir, const TokenizerConfig& config) {
    // tokenizer_config.json
    {
        std::ofstream ofs(dir / "tokenizer_config.json");
        ofs << "{\n";
        ofs << "  \"model_type\": \"llama\",\n";
        ofs << "  \"bos_token\": \"" << config.bos_token << "\",\n";
        ofs << "  \"eos_token\": \"" << config.eos_token << "\",\n";
        ofs << "  \"unk_token\": \"" << config.unk_token << "\",\n";
        ofs << "  \"pad_token\": \"" << config.pad_token << "\",\n";
        ofs << "  \"chat_template\": \"{% for message in messages %}{{ message['content'] }}{% endfor %}\"\n";
        ofs << "}\n";
    }

    // special_tokens_map.json
    {
        std::ofstream ofs(dir / "special_tokens_map.json");
        ofs << "{\n";
        ofs << "  \"bos_token\": \"" << config.bos_token << "\",\n";
        ofs << "  \"eos_token\": \"" << config.eos_token << "\",\n";
        ofs << "  \"unk_token\": \"" << config.unk_token << "\",\n";
