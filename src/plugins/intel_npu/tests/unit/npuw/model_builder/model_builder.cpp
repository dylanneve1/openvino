...
ov::Output<ov::Node> LayerNorm::operator()(const ov::Output<ov::Node>& input,
    ...
    auto bias =
        ov::opset11::Constant::create(precision, ov::Shape{hidden_size}, std::vector<float>(hidden_size, 0.0f));
...
ov::Output<ov::Node> RMSNorm::operator()(const ov::Output<ov::Node>& input,
                                          const std::string& name) const {
    auto weight =
        ov::opset11::Constant::create(precision, ov::Shape{hidden_size}, std::vector<float>(hidden_size, 1.0f));
    weight->set_friendly_name(name + ".weight");
...
std::shared_ptr<ov::Model> ModelBuilder::build_llm(const LLMConfig& config_in) {
    ...
    if (!config.weight)         config.weight = FP32Weight{};
    if (!config.lm_head_weight) config.lm_head_weight = FP32Weight{};
    if (!config.norm)           config.norm = LayerNorm(config.hidden_size, config.precision);
    if (config.position_ids && !config.rope)
        config.rope = HalfRotationRoPE(config.head_dim, 2048, config.precision);
    if (!config.ffn)            config.ffn = SwiGLU(config.hidden_size, config.intermediate_size,
                                                    config.precision, config.weight);
    ...
    for (size_t layer = 0; layer < config.num_layers; ++layer) {
        ...
        SDPAttention attn{config.hidden_size, config.num_heads, config.get_kv_heads(),
                          config.head_dim, config.precision, config.weight, config.rope,
                          config.use_kv_cache, position_ids_output,
                          input_ids->output(0), beam_idx->output(0), sdpa_mask, layer};
