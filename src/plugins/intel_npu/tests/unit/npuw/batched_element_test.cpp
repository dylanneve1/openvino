...
// A zero-sized batch has no rows to score. It must be rejected rather than silently
// producing unpopulated (null) output tensors.
TEST_F(NPUWBatchedElementTest, ZeroBatchIsRejected) {
    auto wrapped = ov::npuw::batched::CompiledModel::create(m_model, m_plugin, make_inner(), /*enabled=*/true);
    auto request = wrapped->create_infer_request();

    ov::Tensor empty(ov::element::f32, ov::Shape{0, K});
    request->set_tensor(wrapped->inputs()[0], ov::get_tensor_impl(empty));

    EXPECT_THROW(request->infer(), ov::Exception);
    EXPECT_EQ(m_recorder->infer_count, 0);
}

// A well-formed [N, K] batch is accepted and produces an [N, K] output. The rejection of an
// input whose batch dimension is neither N nor 1 needs a multi-input model to construct, which
// this single-input fixture cannot express; that path is covered by the assertion in infer().
TEST_F(NPUWBatchedElementTest, WellFormedBatchAccepted) {
    auto wrapped = ov::npuw::batched::CompiledModel::create(m_model, m_plugin, make_inner(), /*enabled=*/true);
    auto request = wrapped->create_infer_request();

    const std::vector<std::vector<float>> rows = {{1, 2, 3, 4}, {5, 6, 7, 8}};
    request->set_tensor(wrapped->inputs()[0], ov::get_tensor_impl(make_input(rows)));
    EXPECT_NO_THROW(request->infer());
    EXPECT_EQ(request->get_tensor(wrapped->outputs()[0])->get_shape(), (ov::Shape{2, K}));
}
