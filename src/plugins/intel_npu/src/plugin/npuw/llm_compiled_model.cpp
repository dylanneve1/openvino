...
ov::npuw::LLMCompiledModel::LLMCompiledModel(const std::shared_ptr<ov::Model>& model,
    ...
    if (prefill_attn_dyn || prefill_attn_pyramid || prefill_attn_hfa || prefill_attn_paged) {
        merge_config_with(prefill_config, dyn_attn_opts);
    }
    if (generate_attn_dyn || generate_attn_pyramid || generate_attn_hfa || generate_attn_paged) {
