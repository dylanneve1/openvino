    ...
    for (std::size_t id = 0u; id < orderedSubgraphs.size(); id++) {
        ...
        if (subgraph._funcall.empty()) {
            ...
            if (compiled_fcn_iter == compiledFunctions.end()) {
                ...
                // with the final tile sub-model — the runtime orchestrator
                // drives the attention compute via tile-loop dispatch and the
                // layer subgraph itself becomes the (single-call) final-tile
                // invocation.
