...
void ov::npuw::CompiledModel::dump_subgraph_model(std::size_t id,
    ...
    // Create dump_subs directory for model files
    const std::string dump_dir = m_cfg.get<::intel_npu::NPUW_DUMP_SUBS_DIR>();
    const std::string subs_dir = dump_dir.empty() ? "dump_subs" : dump_dir + "/dump_subs";
    try {
        ov::util::create_directory_recursive(subs_dir);
    } catch (const std::exception& e) {
        LOG_ERROR("Failed to create directory " << subs_dir << ": " << e.what());
        return;
    }

    const auto model_to_dump = m_compiled_submodels[real_id].model;
    std::string model_dump_path = subs_dir + "/" + m_name + "_" + ov::npuw::util::fmt(id, m_compiled_submodels.size()) +
    ...
    if (m_compiled_submodels[id].pyramid_attention) {
        ...
        for (std::size_t idx = 0; idx < pyramid_attention_models.size(); ++idx) {
            std::string pyramid_attention_model_dump_path =
                subs_dir + "/" + m_name + "_" + ov::npuw::util::fmt(id, m_compiled_submodels.size()) +
    ...
    if (m_compiled_submodels[id].host_flash_attention) {
        ...
        std::string hfa_tile_model_dump_path = subs_dir + "/" + m_name + "_" + ov::npuw::util::fmt(id, m_compiled_submodels.size()) +
        ...
        std::string hfa_final_tile_model_dump_path = subs_dir + "/" + m_name + "_" +
...
void ov::npuw::CompiledModel::dump_subgraph_composition(const std::vector<ov::npuw::Subgraph>& orderedSubgraphs) const {
    ...
    for (size_t i = 0; i < subgraph_order.size(); i++) {
        json_file << "    \"./dump_subs/" << subgraph_order[i] << ".xml\"";
