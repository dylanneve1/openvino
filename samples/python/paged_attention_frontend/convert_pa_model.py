...
"""Produce a PagedAttention IR from a stateful text-generation IR.
...
the resulting model carries the exact input contract Stage 0 targets.
...
from paged_front_end import is_paged_attention_model, to_paged_attention
...
def main() -> int:
    ...
    if is_paged_attention_model(model):
        print(f"{args.src} is already a PagedAttention model; copying signature.")
    else:
        print("Applying paged_attention_transformation ...")
        to_paged_attention(model)

    print("\nPagedAttention model inputs:")
    kv_layers = 0
